...
# Final comprehensive mobile menu structure fix

echo "🔧 Final mobile menu structure fix..."
...
fix_file() {
    ...
    # Step 1: Remove any existing mobile menu content that's showing on desktop
    sed -i '/<div class="px-4 py-4 space-y-2">/,/<\/div>\s*<\/div>\s*<\/div>/d' "$file"
    
    # Step 2: Remove any malformed closing tags
    sed -i 's/<!-- Mobile Menu Button -->\s*<\/a>/<!-- Mobile Menu Button -->/g' "$file"
    
    # Step 3: Add proper mobile menu structure before closing nav
    ...
    # Step 4: Fix any remaining malformed tags
    sed -i 's/<\/button>\s*<\/a>/<\/button>/g' "$file"
...
verify_fix() {
    ...
    if [ "$mobile_menu_count" -eq 1 ] && [ "$hidden_class_count" -eq 1 ] && [ "$mobile_button_count" -eq 1 ]; then
        echo "✅ Verified: $file - Mobile menu looks perfect"
    else
        echo "❌ Warning: $file - Issues detected (Mobile menu: $mobile_menu_count, Hidden class: $hidden_class_count, Mobile button: $mobile_button_count)"
    fi
}

# Main execution
echo "🚀 Starting final mobile structure fix..."
