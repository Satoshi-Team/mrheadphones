#!/bin/bash

echo "🔧 Making studying page navigation match homepage..."

# Replace the entire navigation section in the studying page to match homepage
sed -i '/<!-- Navigation -->/,/<!-- Mobile Menu Button -->/c\
    <!-- Navigation -->\
    <nav class="bg-white dark:bg-gray-900 shadow-lg sticky top-0 z-50 transition-colors duration-300">\
        <div class="content-container-wide">\
            <div class="flex items-center justify-between py-4">\
                <!-- Logo -->\
                <a href="/" class="flex items-center space-x-2 group">\
                    <img src="/public/favicon.svg?v=1.0.1" alt="MrHeadphones Logo" class="w-6 h-6 sm:w-7 sm:h-7 lg:w-8 lg:h-8 transition-all duration-300 group-hover:scale-105">\
                    <span class="font-heading font-bold text-base sm:text-lg">MrHeadphones</span>\
                </a>\
                \
                <!-- Desktop Menu - Centered -->\
                <div class="hidden md:flex items-center justify-center flex-1 mx-8">\
                    <div class="flex items-center space-x-8">\
                        <a href="/#home" class="nav-link" data-translate="nav_home">Home</a>\
                        <a href="/#categories" class="nav-link" data-translate="nav_categories">Categories</a>\
                        <a href="/#devices" class="nav-link" data-translate="nav_devices">Devices</a>\
                        <a href="/#solutions" class="nav-link" data-translate="nav_solutions">Solutions</a>\
                        <a href="/#budget" class="nav-link" data-translate="nav_budget">Budget</a>\
                    </div>\
                </div>\
                \
                <!-- Right Side Controls -->\
                <div class="flex items-center space-x-2">\
                    <!-- Dark Mode Toggle -->\
                    <button id="darkModeToggle" aria-label="Toggle dark mode" data-translate-title="dark_mode_toggle" class="p-3 rounded-2xl hover:bg-gray-100/60 dark:hover:bg-gray-700/60 transition-all duration-300 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-secondary/30">\
                        <span class="dark:hidden">\
                            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">\
                                <path d="M17.293 13.293A8 8 0 016.707 2.707a8.001 8.001 0 1010.586 10.586z"/>\
                            </svg>\
                        </span>\
                        <span class="hidden dark:inline">\
                            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">\
                                <path fill-rule="evenodd" d="M10 2a1 1 0 011 1v1a1 1 0 11-2 0V3a1 1 0 011-1zm4 8a4 4 0 11-8 0 4 4 0 018 0zm-.464 4.95l.707.707a1 1 0 001.414-1.414l-.707-.707a1 1 0 00-1.414 1.414zm2.12-10.607a1 1 0 010 1.414l-.706.707a1 1 0 11-1.414-1.414l.707-.707a1 1 0 011.414 0zM17 11a1 1 0 100-2h-1a1 1 0 100 2h1zm-7 4a1 1 0 011 1v1a1 1 0 11-2 0v-1a1 1 0 011-1zM5.05 6.464A1 1 0 106.465 5.05l-.708-.707a1 1 0 00-1.414 1.414l.707.707zm1.414 8.486l-.707.707a1 1 0 01-1.414-1.414l.707-.707a1 1 0 011.414 1.414zM4 11a1 1 0 100-2H3a1 1 0 000 2h1z" clip-rule="evenodd"/>\
                            </svg>\
                        </span>\
                    </button>\
                </div>\
                \
                <!-- Mobile Menu Button -->\
                <button id="mobileMenuToggle" class="md:hidden p-2 rounded-lg bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors">\
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">\
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"></path>\
                    </svg>\
                </button>\
            </div>\
            \
            <!-- Mobile Menu -->\
            <div id="mobileMenu" class="md:hidden hidden pb-4">\
                <div class="flex flex-col space-y-4">\
                    <a href="/#home" class="mobile-nav-link" data-translate="nav_home">Home</a>\
                    <a href="/#categories" class="mobile-nav-link" data-translate="nav_categories">Categories</a>\
                    <a href="/#devices" class="mobile-nav-link" data-translate="nav_devices">Devices</a>\
                    <a href="/#solutions" class="mobile-nav-link" data-translate="nav_solutions">Solutions</a>\
                    <a href="/#budget" class="mobile-nav-link" data-translate="nav_budget">Budget</a>\
                </div>\
            </div>\
        </div>\
    </nav>' categories/best-headphones-for-studying/index.html

echo "✅ Studying page navigation now matches homepage!" 