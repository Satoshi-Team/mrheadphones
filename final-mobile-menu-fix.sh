...
echo "🔧 Final comprehensive mobile menu fix..."

# Function to completely rebuild a page's navigation
fix_page_navigation() {
    local file="$1"
    ...
    # Get the title and description from the head
    local title=$(grep '<title>' "$file" | sed 's/.*<title>\(.*\)<\/title>.*/\1/')
    local description=$(grep '<meta name="description"' "$file" | sed 's/.*content="\([^"]*\)".*/\1/')
...
<head>
    ...
    <!-- Favicon -->
    <link rel="icon" type="image/svg+xml" href="/public/favicon.svg?v=1.0.3">
    <!-- Google tag (gtag.js) -->
...
EOF
    ...
    # Now append the rest of the content from the backup, starting after the body tag
    sed -n '/<body/,/<\/body>/p' "${file}.backup4" | sed '1d;$d' | sed '/<!-- Navigation -->/,/<\/nav>/d' >> "$file"
...
# List of problematic pages that need complete navigation rebuild
problematic_pages=(
...
for page in "${problematic_pages[@]}"; do
    if [ -f "$page" ]; then
        fix_page_navigation "$page"
    fi
done

echo "🎉 Final mobile menu fixes completed!" 